...
def func(number_of_sims: int, file_name: str):
    ...
    with open(file_name, "w") as file:
        ...
        header_1 = ['states in environment: ' + str(10)]
        ...
        for i in range(1):            
            ...
            while s < number_of_sims:
                ...
                trajectory = Markov_flex_sys_flex_env(np.array([[.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2]]), generate_random_matrix(2), generate_random_matrix(2), p_t3= generate_random_matrix(2), p_t4= generate_random_matrix(2), p_t5= generate_random_matrix(2), p_t6= generate_random_matrix(2), p_t7 = generate_random_matrix(2), p_t8 =generate_random_matrix(2), p_t9 =generate_random_matrix(2), p_t99 =generate_random_matrix(2)) # abritrary system states here 
                ...
                for sys_size in range (2, 11):
                    trajectory = Markov_flex_sys_flex_env(np.array([[.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2], [.2, .2, .2, .2, .2]]), generate_random_matrix(sys_size), generate_random_matrix(sys_size), p_t3= generate_random_matrix(sys_size), p_t4= generate_random_matrix(sys_size), p_t5= generate_random_matrix(sys_size), p_t6= generate_random_matrix(sys_size), p_t7= generate_random_matrix(sys_size), p_t8 =generate_random_matrix(sys_size), p_t9=generate_random_matrix(sys_size), p_t99=generate_random_matrix(sys_size))
                    trajectory.generate_ensemble(10000)
                    trajectory.calculate_probs()
...
file_name = "8_3_10_state_set_3" + sys.argv[1] + ".csv"
