...
def small_sim_env_size_5(number_of_sims: int, file_name: str):
    """
    for set 1: 
        Broken DB
        weak coupling
    ...
    with open(file_name, "w") as file:
        ...
        trajectories = []
        randomly_generated_env_size_x = [generate_random_matrix(5) for i in range(10)]
        for s in range(10):
            writer.writerow(["example: " + str(s)])
            for i in range(3, 6):
                writer.writerow(["states in system: " + str(i)])
                trajectories = [Markov_flex_sys_flex_env(randomly_generated_env_size_x[s], generate_random_matrix(i), generate_random_matrix(i), p_t3= generate_random_matrix(i), p_t4= generate_random_matrix(i), p_t5= generate_random_matrix(i)) for x in range(number_of_sims)]
                I_pred_max_one_list = []
                stored_info_list = []
...
small_sim_env_size_5(1000, "small_sim_env_size_5.csv")
print("hi")