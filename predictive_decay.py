...
test_1 = Markov_three(p_transition_env = np.array([[.05, .9, .05], [.05, .05, .9], [.9, .05, .05]]),
...
test_1_env.calculate_probs_pred()
...
plt.title("information decay starting at time step 50")
plt.plot(x_axis, coupled_decay, label = 'I[s_t, x_k]')
plt.plot(x_axis, env_decay[50:100], label = 'I[x_t, x_k]')
plt.legend()
plt.show()
...
plt.xlabel("time steps after t")
plt.ylabel("information (nats)")
plt.title("zoomed in- information decay starting at time step 50")
plt.plot(x_axis[1:25], coupled_decay[1:25], label = 'I[s_t, x_k]')
plt.plot(x_axis[1:25], env_decay[51:75], label = 'I[x_t, x_k]')
plt.legend()
plt.show()

print(test_1_env.cw_to_ccw_ratio())
print(test_1_env.matrix)
print(test_1_env.entropy)
